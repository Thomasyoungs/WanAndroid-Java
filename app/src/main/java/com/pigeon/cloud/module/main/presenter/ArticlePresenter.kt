...
class ArticlePresenter : BasePresenter<ArticleView>() {
    ...
    fun collect() {
        addToRxLife(MainRequest.collectArticle(articleId, object :
    ...
    fun uncollect() {
        addToRxLife(MainRequest.uncollectArticle(articleId, object :
            RequestListener<BaseBean?> {
            override fun onStart() {}
    ...
    fun updateReadRecordPercent(link: String, percent: Float) {
        ...
        mReadRecordExecutor?.updatePercent(link, percent, lastTime, {
            val readRecordUpdateEvent =
                ReadRecordUpdateEvent(it.link)
